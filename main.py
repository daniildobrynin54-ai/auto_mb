...
class MangaBuffApp:
    ...
    def enter_wait_mode(self, current_boost_card: dict) -> None:
        """
        🔧 ИСПРАВЛЕНО: Режим ожидания НЕ парсит карты.
        ...
        2. Мониторинг работает (проверка страницы буста)
        3. Telegram бот активен
        4. История обменов обновляется
        """
        # 🔧 КРИТИЧНО: Отменяем все обмены ПЕРЕД входом в режим ожидания
        ...
        while True:
            ...
            # Проверка смены карты (мониторинг работает в фоне)
            ...
            if self.replace_requested:
                ...
                if new_card:
                    ...
                    # Продолжаем ожидание с новой картой
                else:
